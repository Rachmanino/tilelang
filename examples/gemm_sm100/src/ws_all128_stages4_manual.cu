...
extern "C" __global__ void __launch_bounds__(128, 1) gemm_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, __grid_constant__ const CUtensorMap C_desc) {
  ...
  if ((32 <= ((int)threadIdx.x)) && (((int)threadIdx.x) < 64)) {
    for (int k = 0; k < 32; ++k) {
      mbarrier[((k % 4) + 4)].wait((((k % 8) / 4) ^ 1));
      if (tl::tl_shuffle_elect<32>()) {
        mbarrier[(k % 4)].expect_transaction(32768);
        tl::fence_proxy_async();
        tl::tma_load(A_desc, mbarrier[(k % 4)], (&(((bfloat16_t*)buf_dyn_shmem)[((k % 4) * 16384)])), (k * 128), (((int)blockIdx.y) * 128));
        tl::tma_load(A_desc, mbarrier[(k % 4)], (&(((bfloat16_t*)buf_dyn_shmem)[(((k % 4) * 16384) + 8192)])), ((k * 128) + 64), (((int)blockIdx.y) * 128));
        mbarrier[(k % 4)].expect_transaction(32768);
        tl::fence_proxy_async();
        tl::tma_load(B_desc, mbarrier[(k % 4)], (&(((bfloat16_t*)buf_dyn_shmem)[(((k % 4) * 16384) + 65536)])), (((int)blockIdx.x) * 128), (k * 128));
        tl::tma_load(B_desc, mbarrier[(k % 4)], (&(((bfloat16_t*)buf_dyn_shmem)[(((k % 4) * 16384) + 65536+8192)])), ((((int)blockIdx.x) * 128) + 64), (k * 128));
      }
      mbarrier[(k % 4)].arrive();
    }
  } else if (((int)threadIdx.x) < 32) {
    for (int k = 0; k < 32; ++k) {
        mbarrier[(k % 4)].wait(((k % 8) / 4));
        tl::initialize_tcgen05_descriptor(desc_a, (&(((bfloat16_t*)buf_dyn_shmem)[(k%4)*16384])), 1, 64, 0, 0, 2);
        tl::initialize_tcgen05_descriptor(desc_b, (&(((bfloat16_t*)buf_dyn_shmem)[(k%4)*16384+65536])), 1024, 64, 0, 0, 2);
        #pragma unroll
        ...
        tl::tcgen05_mma_arrive((&(mbarrier[((k % 4) + 4)])));
        mbarrier[((k % 4) + 4)].wait((k%8)/4);
    }
  }
  __syncthreads();
  ...
  if (((int)threadIdx.x) == 0) {
    ...
    tl::tma_store_arrive();
    tl::tma_store_wait<0>();
    tl::tma_store(C_desc, (&(((bfloat16_t*)buf_dyn_shmem)[65536])), ((((int)blockIdx.x) * 128) + 64), (((int)blockIdx.y) * 128));
    tl::tma_store_arrive();
    tl::tma_store_wait<0>();
  }
  if ((((int)threadIdx.x) >> 5) == 0) {
