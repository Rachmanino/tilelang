...
def gemm(
    ...
    with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
        ...
        give_me_more_smem = T.alloc_shared((block_M, block_N), out_dtype)
        ...
        T.copy(C_shared, give_me_more_smem)  # avoid Dead Code Elimination
        T.copy(give_me_more_smem, C_shared)
...
def tilelang_callback_c_postproc(code, _):
    ...
    # with open("gemm_tcgen5mma_tma_host_code.c", "w") as f:
    #     print(code, file=f)
...
def tilelang_callback_cuda_postproc(code, _):
    ...
    code = r"""
// Manual stages=4 implementation
// ~ 1150 tflops
// smem size ~ 32768KiB x 6 
...
extern "C" __global__ void __launch_bounds__(128, 1) gemm_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, __grid_constant__ const CUtensorMap C_desc) {
  ...
  if ((((int)threadIdx.x) >> 5) == 0) {
    for (int k = 0; k < 64; ++k) {
      mbarrier[((k % 6) + 6)].wait((((k % 12) / 6) ^ 1));
      if (cute::elect_one_sync()) {  // seems to be faster than tl::tl_shuffle_elect<32>()
        mbarrier[k % 6].expect_transaction(16384);
        tl::tma_load(A_desc, mbarrier[k % 6], (&(((bfloat16_t*)buf_dyn_shmem)[((k % 6) * 8192)])), (k * 64), (((int)blockIdx.y) * 128));
        mbarrier[k % 6].expect_transaction(16384);
        tl::tma_load(B_desc, mbarrier[k % 6], (&(((bfloat16_t*)buf_dyn_shmem)[((k % 6) * 8192 + 49152)])), (((int)blockIdx.x) * 128), (k * 64));
        tl::tma_load(B_desc, mbarrier[k % 6], (&(((bfloat16_t*)buf_dyn_shmem)[((k % 6) * 8192 + 53248)])), ((((int)blockIdx.x) * 128) + 64), (k * 64));
      }
      mbarrier[k % 6].arrive();
    }
  } else if ((((int)threadIdx.x) >> 5) == 1) {
    for (int k = 0; k < 64; ++k) {
        mbarrier[(k % 6)].wait(((k % 12) / 6));
        tl::initialize_tcgen05_descriptor(desc_a, (&(((bfloat16_t*)buf_dyn_shmem)[(k % 6) * 8192])), 1, 64, 0, 0, 2);
        tl::initialize_tcgen05_descriptor(desc_b, (&(((bfloat16_t*)buf_dyn_shmem)[(k % 6 + 6) * 8192])), 512, 64, 0, 0, 2);
        #pragma unroll
        for (int ki = 0; ki < 4; ++ki) {
          tl::tcgen05mma_ss<tl::DataType::kBFloat16>(uint64_t(desc_a + (ki * 32)), uint64_t(desc_b + (ki * 2048)), (*reinterpret_cast<uint32_t*>(C_tmem)) + 0, ((0 < ki) ? 1 : ((k == 0) ? 0 : 1)), static_cast<uint32_t>(136381584), 0, 0, 0, 0);
        }
        tl::tcgen05_mma_arrive((&(mbarrier[((k % 6) + 6)])));
    }
  }
  // wait for consumers in the last round to finish
  mbarrier[6].wait(0);  // flipped 11 times
  mbarrier[7].wait(0);
  mbarrier[8].wait(0);
  mbarrier[9].wait(0);
  mbarrier[10].wait(1);  // flipped 10 times
  mbarrier[11].wait(1);
  __syncthreads();
...
num_stages = 6  # should align with the number of stages in CUDA
threads = 128
...
if torch.allclose(c, ref_c, rtol=1e-2, atol=1e-2):
